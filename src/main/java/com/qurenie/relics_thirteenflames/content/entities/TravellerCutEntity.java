...
public class TravellerCutEntity extends NonLivingEntity implements IAnimatedEntity {
    ...
    public TravellerCutEntity(EntityType<? extends LivingEntity> entityType, Level level, Player owner, ItemStack stack, int fireAspect) {
        super(entityType, level);
        this.owner = owner;
        this.stack = stack;
    ...
    private List<Float> getAngles() {
        ...
        final float rot = 1f / (ANIM_LENGTH - PRE_ANIM_LENGTH);
    ...
    public void tick() {
        ...
        if (level().isClientSide) {
            if (tickCount > PRE_ANIM_LENGTH - 4)
        ...
        if (tickCount >= PRE_ANIM_LENGTH)
            setCompletion(getCompletion() + 1f / (ANIM_LENGTH - PRE_ANIM_LENGTH));
    }
