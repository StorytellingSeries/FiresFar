...
public class JodahHealEntity extends Entity {
    ...
    public static final String GAIN_SHIELD = "gain_shield";
    ...
    private static final OctoColor PURPLE_COLOR = FlamesUtils.fromRGBI(160, 20, 120);
    float healingPower;
    int age = 0;
    int xpPower;
    @Getter
...
//
        ...
        if (this.level().isClientSide) {
            ...
            if (Math.random() * 1.5 > tickCount % 3)
                ParticleHelper.spawnParticleLine(level(), ParticleHelper.constructFigure(new OctoColor(0xFFAAAAAA), (float) (0.15f + level().random.nextDouble() * 0.09f),
        ...
        if (this.distanceToSqr(getOwnerHeardPos()) <= SUCK_IN_RADIUS) {
            if (owner.getHealth() >= owner.getMaxHealth() && gainShield)
                FlamesUtils.gainJodahShield(owner, healingPower);
            else
    ...
    protected Vec3 getRawMovement() {
        ...
        } else if (target != null && target.isAlive() && this.distanceToSqr(getTargetHeardPos()) <= RADIUS * RADIUS) {
    ...
    protected void readAdditionalSaveData(@NotNull CompoundTag compound) {
        ...
        this.gainShield = compound.getBoolean(GAIN_SHIELD);
    ...
    protected void addAdditionalSaveData(@NotNull CompoundTag compound) {
        ...
        compound.putBoolean(GAIN_SHIELD, gainShield);
    }
