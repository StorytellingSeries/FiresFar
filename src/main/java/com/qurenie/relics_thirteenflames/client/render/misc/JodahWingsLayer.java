...
//    private EntityAnimatedWrapper livingWrapper;
    private final Map<LivingEntity, EntityAnimatedWrapper> wrappers = new WeakHashMap<>();
    private final Map<LivingEntity, Integer> lastTicks = new WeakHashMap<>();
    ...
    public void clientTick(PlayerTickEvent.Pre event) {
        ...
        if (activeTicks == 0) {
            // крылья деактивированы — закрываем и чистим, если обёртка была
            EntityAnimatedWrapper wrapper = wrappers.remove(player);
            if (wrapper != null)
                wrapper.close();
        ...
        boolean isNewActivation = !wrappers.containsKey(player);
        EntityAnimatedWrapper wrapper = wrappers.computeIfAbsent(player, EntityAnimatedWrapper::new);

        // Устойчивое сравнение вместо ==: ловим момент пересечения порога,
        // даже если тик "перепрыгнул" значение из-за лага/десинхронизации.
        int prevTicks = lastTicks.getOrDefault(player, activeTicks - 1);
        lastTicks.put(player, activeTicks);

        if (isNewActivation || crossed(prevTicks, activeTicks, ANIMATION_LENGTH)) {
            wrapper.getAnimationSystem().startAnimationAt(LAYER_ACTION,
                    AnimationsRegistry.JODAH_WINGS_OPEN.configure().important().speed(0.8f));
        } else if (crossed(prevTicks, activeTicks, 50)) {
            wrapper.getAnimationSystem().startAnimationAt(LAYER_ACTION, AnimationsRegistry.JODAH_WINGS_OPEN
                    .configure().reversed().important().speed(0.8f).startTime(1.2f).transitionTime(0.4f));
        } else if (crossed(prevTicks, activeTicks, ANIMATION_LENGTH - 27)) {
            spawnClosingParticlesSmall(player);
        } else if (crossed(prevTicks, activeTicks, ANIMATION_LENGTH - 50)) {
            spawnClosingParticlesBig(player);
        }
    }

    private static boolean crossed(int prev, int curr, int threshold) {
        return prev > threshold && curr <= threshold;
    }

    private void spawnClosingParticlesSmall(LivingEntity player) {
        ...
        ParticleHelper.spawnParticleTriangle(player.level(), new FeatherParticle.Options(0.3f, 50),
                corners[2], corners[2].add(left.normalize().scale(-1)).add(0, 0.7, 0), corners[0].add(0, 0.6, 0), 3,
                () -> right.add(random.nextGaussian() * 0.07, random.nextGaussian() * 0.1, random.nextGaussian() * 0.07).normalize().scale(0.15));
        ParticleHelper.spawnParticleTriangle(player.level(), new FeatherParticle.Options(0.3f, 50),
                corners[3], corners[3].add(right.normalize().scale(-1)).add(0, 0.7, 0), corners[1].add(0, 0.6, 0), 3,
                () -> left.add(random.nextGaussian() * 0.07, random.nextGaussian() * 0.1, random.nextGaussian() * 0.07).normalize().scale(0.15));
    }

    private void spawnClosingParticlesBig(LivingEntity player) {
        Vec3[] corners = getWingCorners(player);
        double rad = Math.toRadians(player.yBodyRot);
        Vec3 right = new Vec3(Math.cos(rad), 0, Math.sin(rad)).normalize().scale(0.2f);
        Vec3 left = right.scale(-1);
        ...
        ParticleHelper.spawnParticleTriangle(player.level(), new FeatherParticle.Options(0.3f, 125),
                corners[2], corners[2].add(left.normalize().scale(-1)).add(0, 1.4, 0), corners[0].add(0, 0.6, 0), 4,
                () -> right.add(random.nextGaussian() * 0.3, random.nextGaussian() * 0.14 + 0.5, random.nextGaussian() * 0.3).normalize().scale(0.15));
        ParticleHelper.spawnParticleTriangle(player.level(), new FeatherParticle.Options(0.3f, 125),
                corners[3], corners[3].add(right.normalize().scale(-1)).add(0, 1.4, 0), corners[1].add(0, 0.6, 0), 4,
                () -> left.add(random.nextGaussian() * 0.3, random.nextGaussian() * 0.14 + 0.5, random.nextGaussian() * 0.3).normalize().scale(0.15));
    }
    ...
    public void render(@NotNull PoseStack poseStack, @NotNull MultiBufferSource bufferSource, int packedLight,
        ...
        EntityAnimatedWrapper wrapper = wrappers.get(livingEntity);
        if (activeTicks == 0 || wrapper == null)
            return;

        // Перенесено сюда из clientTick: свойства должны копироваться на каждый кадр
        // для конкретной рендерящейся сущности, а не раз в тик для "текущего" игрока.
        ...
        wingsLayer.applyAnimations(wrapper.getAnimationSystem(), partialTick);
